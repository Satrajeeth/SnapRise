...
async def get_boards(db: AsyncSession = Depends(get_db_session)):
    return await BoardOps.get_boards(db)

@router.get("/boards/{board_id}", response_model=BoardDetailed)
async def get_board(board_id: UUID, db: AsyncSession = Depends(get_db_session)):
...
async def create_board(board_in: BoardCreate, db: AsyncSession = Depends(get_db_session)):
    return await BoardOps.create_board(db, board_in)

@router.patch("/boards/{board_id}", response_model=Board)
async def update_board(board_id: UUID, board_in: BoardUpdate, db: AsyncSession = Depends(get_db_session)):
...
async def delete_board(board_id: UUID, db: AsyncSession = Depends(get_db_session)):
...
async def create_column(column_in: ColumnCreate, db: AsyncSession = Depends(get_db_session)):
    return await BoardOps.create_column(db, column_in)

@router.patch("/columns/{column_id}", response_model=Column)
async def update_column(column_id: UUID, column_in: ColumnUpdate, db: AsyncSession =Depends(get_db_session)):
...
async def delete_column(column_id: UUID, db: AsyncSession = Depends(get_db_session)):
...
async def create_task(task_in: TaskCreate, db: AsyncSession = Depends(get_db_session)):
    return await BoardOps.create_task(db, task_in)

@router.patch("/tasks/{task_id}", response_model=Task)
async def update_task(task_id: UUID, task_in: TaskUpdate, db: AsyncSession = Depends(get_db_session)):
...
async def create_subtask(subtask_in: SubtaskCreate, db: AsyncSession = Depends(get_db_session)):
    return await BoardOps.create_subtask(db, subtask_in)

@router.patch("/subtasks/{subtask_id}", response_model=Subtask)
async def update_subtask(subtask_id: UUID, subtask_in: SubtaskUpdate, db: AsyncSession = Depends(get_db_session)):
...
async def delete_subtask(subtask_id: UUID, db: AsyncSession = Depends(get_db_session)):
...
async def analyze_task_ai(task_id: UUID, db: AsyncSession = Depends(get_db_session)):
    ...
    blockers = await ai_service.detect_blockers(task.title, task.content, subtask_tiles)
...
async def classify_task_ai(task_id: UUID, db: AsyncSession = Depends(get_db_session)):
